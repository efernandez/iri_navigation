...
algorithm_base::IriBaseAlgorithm<Sensors2LinkAlgorithm>()
{
  ...
  public_node_handle_.param<bool>("online_mode_", online_mode_, true);
    ...
    {  
      ...
      if (get_relative_pose_client_.call(get_relative_pose_srv_) && get_relative_pose_srv_.response.success)
      {
        ...
        if (!is_semiPD(covariance_2_matrix(res.odom.pose)))
          std::cout << "ERROR: Loop closure ICP_cov is not PD!" << std::endl << covariance_2_matrix(res.odom.pose) << std::endl;
      }
      else
      {
...
void Sensors2LinkAlgNode::load_tf()
{
  ...
  try
  {
    tfl.waitForTransform(base_frame_id_, laser_frame_id_, ros::Time::now(), ros::Duration(10.0));
